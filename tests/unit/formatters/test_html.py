...
import cogni
from cogni.core import config
from cogni.core import issue
from cogni.core import manager
from cogni.formatters import html as b_html
...
class HtmlFormatterTests(testtools.TestCase):
    ...
    def test_report_with_skipped(self):
        ...
        with open(self.tmp_fname, "w") as tmp_file:
            b_html.report(self.manager, tmp_file, cogni.LOW, cogni.LOW)
    ...
    def test_report_contents(self, get_issue_list, get_code):
        ...
        issue_a = _get_issue_instance(severity=cogni.LOW)
        ...
        issue_b = _get_issue_instance(severity=cogni.MEDIUM)
        issue_c = _get_issue_instance(severity=cogni.HIGH)
...
def _get_issue_instance(
    severity=cogni.MEDIUM, cwe=123, confidence=cogni.MEDIUM
):
