...
import cogni
from cogni.core import constants
from cogni.core import issue
...
class IssueTests(testtools.TestCase):
    ...
    def test_issue_filter_severity(self):
        levels = [cogni.LOW, cogni.MEDIUM, cogni.HIGH]
        issues = [_get_issue_instance(level, cogni.HIGH) for level in levels]

        for level in levels:
            rank = constants.RANKING.index(level)
            for i in issues:
                test = constants.RANKING.index(i.severity)
                result = i.filter(level, cogni.UNDEFINED)
                self.assertTrue((test >= rank) == result)

    def test_issue_filter_confidence(self):
        levels = [cogni.LOW, cogni.MEDIUM, cogni.HIGH]
        issues = [_get_issue_instance(cogni.HIGH, level) for level in levels]

        for level in levels:
            rank = constants.RANKING.index(level)
            for i in issues:
                test = constants.RANKING.index(i.confidence)
                result = i.filter(cogni.UNDEFINED, level)
                self.assertTrue((test >= rank) == result)

    def test_matches_issue(self):
        issue_a = _get_issue_instance()

        issue_b = _get_issue_instance(severity=cogni.HIGH)

        issue_c = _get_issue_instance(confidence=cogni.LOW)
    ...
    def test_get_code(self, getline):
        ...
        new_issue = issue.Issue(
            cogni.MEDIUM, cwe=issue.Cwe.MULTIPLE_BINDS, lineno=1
        )
...
def _get_issue_instance(
    severity=cogni.MEDIUM,
    cwe=issue.Cwe.MULTIPLE_BINDS,
    confidence=cogni.MEDIUM,
):
